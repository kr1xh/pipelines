...
export function getArtifactsHandler({
  ...
  return async (req, res) => {
    ...
    // Use bucket-specific validation for S3/Minio buckets (allows dots)
    // Use resource name validation for volume sources (Kubernetes naming rules)
    const isValidBucketName = source === 'volume' ? isAllowedResourceName(bucket) : isAllowedBucketName(bucket);
